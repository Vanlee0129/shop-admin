  ...
  <div class="content-page">
    <div class="content-nav">
      <el-breadcrumb class="breadcrumb" separator="/">
        <el-breadcrumb-item :to="{ name: 'freight' }"
          >运费模板</el-breadcrumb-item
        >
        <el-breadcrumb-item>{{
          infoForm.id ? "编辑模板" : "添加模板"
        }}</el-breadcrumb-item>
      </el-breadcrumb>
      ...
      <div class="form-table-box">
        ...
        >
                ...
                >
                  <template slot-scope="scope">
                    <el-switch
                      ...
                      active-color="#13ce66"
                      @change="
                        changeDefaultNumberSwitch(scope.$index, scope.row)
                      "
                    >
                    </el-switch>
                    <el-input-number
                      ...
                      placeholder="件"
                      @blur="
                        changeDefaultNumberInput(
                          scope.$index,
                          scope.row.free_by_number
                        )
                      "
                      @change="
                        changeDefaultNumberInput(
                ...
                >
                  <template slot-scope="scope">
                    <el-switch
                      ...
                      active-color="#13ce66"
                      @change="
                        changeDefaultMoneySwitch(scope.$index, scope.row)
                      "
                    >
                    </el-switch>
                    <el-input-number
                      ...
                      placeholder="金额"
                      @blur="
                        changeDefaultMoneyInput(
                          scope.$index,
                          scope.row.free_by_money
                        )
                      "
                      @change="
                        changeDefaultMoneyInput(
          ...
          <el-form-item label="指定区域运费" class="special-freight">
                ...
                >
                  <template slot-scope="scope">
                    ...
                    <el-input-number
                      ...
                      placeholder="金额"
                      @blur="
                        changeMoneyInput(scope.$index, scope.row.free_by_money)
                      "
                      @change="
                        changeMoneyInput(scope.$index, scope.row.free_by_money)
                      "
            ...
            <div class="add-btn">
              <el-button type="text" @click="add_template"
                >+添加指定地区运费</el-button
              >
              <el-form-item>
                <el-button
...
export default {
  ...
  methods: {
    ...
    updateArea() {
      ...
      for (const item in areaName) {
        all.map((ele) =>
          ele.id == areaName[item] ? newName.push(ele.name) : ""
        );
      }
