          ...
          <div class="content-wrap clearfix">
            ...
            <div class="right">
              <el-button
                class="right-detail"
                type="text"
                @click="viewDetail(item.order_sn)"
                size="mini"
              >
                查看详情
              </el-button>
      ...
      <div class="address-wrap" v-if="this.pIndex == 1">
        <div class="coupon-w">
          ...
          <div class="bottom" v-for="(item, index) in addressData" :key="index">
            ...
            <div class="r">
              <el-button size="small" @click="addressEdit(item)"
                >编辑</el-button
              >
            </div>
          </div>
...
export default {
  ...
  methods: {
    saveAddress() {
      ...
      this.axios
        .post("user/saveaddress", this.nowAddressData)
        .then((response) => {
    ...
    submitName(index, row) {
      this.axios
        .post("user/updateName", { id: row.id, name: row.name })
        .then((response) => {
          if (response.data.errno === 0) {
            this.$message({
              type: "success",
    ...
    submitMobile(index, row) {
      this.axios
        .post("user/updateMobile", { id: row.id, mobile: row.mobile })
        .then((response) => {
          if (response.data.errno === 0) {
            this.$message({
              type: "success",
