      ...
      <div class="form-table-box">
        ...
        >
          <el-form-item label="打印后自动发货">
            <el-switch
              v-model="infoForm.autoDelivery"
              @change="changeStatus(infoForm.autoDelivery)"
            ></el-switch>
          </el-form-item>
  ...
  methods: {
    ...
    changeStatus() {
      this.infoForm.autoDelivery == true
        ...
        : (this.infoForm.autoDelivery = 0);
      this.axios
        .post("admin/changeAutoStatus", { status: this.infoForm.autoDelivery })
        .then((response) => {
    ...
    onSaveSubmit() {
      ...
      this.$refs["infoForm"].validate((valid) => {
        if (valid) {
          this.axios
            .post("admin/storeShipperSettings", this.infoForm)
            .then((response) => {
              if (response.data.errno === 0) {
                this.$message({
                  type: "success",
